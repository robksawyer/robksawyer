...
describe('components', () => {
  describe('IconLinkedIn', () => {
    it('should mount with props', function () {
      const wrap = mount(<IconLinkedIn>Hello World</IconLinkedIn>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<IconLinkedIn>Hello World</IconLinkedIn>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('icon_linked_in')
